...
require(DESeq2) # for the DE hypothesis testing
require(psych) # for calculating geometric mean
require(PoissonSeq) # for PoissonSeq normalization
require(truncnorm) # for generating sequencing depths
require(edgeR) # for TMM normalization
require(TCC) # for DEGES normalization
...
for(k in percDE){
  ...
  for(j in 1:nsamp){
    ...
    if (symmetric){
      ...
      baselineprop1[i1] <- baselineprop1[i1]*2
      baselineprop2[i2] <- baselineprop2[i2]*4
    }
...
write.table(efdrMat, "efdrMatPartialAsymmSameMRNA_prm2.txt")
...
write.table(efdrSDMat, "efdrSDMatPartialAsymmSameMRNA_prm2.txt")
...
write.table(powerMat, "powerMatPartialAsymmSameMRNA_prm2.txt")
...
write.table(powerSDMat, "powerSDMatPartialAsymmSameMRNA_prm2.txt")

MSEMat <- data.frame(MSEMat)
colnames(MSEMat) <- 
  c("Oracle", "Total Count", "DESeq", "edgeR", "TCC", "PoissonSeq")

write.table(MSEMat, "MSEMatPartialAsymmSameMRNA_prm2.txt")
