...
n1 <- 5 # num samples in first cond.
n2 <- 5 # num samples in second cond.
...
write.table(efdrMat, "efdrMatSymmSameMRNA.txt")
...
write.table(efdrSDMat, "efdrSDMatSymmSameMRNA.txt")
...
write.table(powerMat, "powerMatSymmSameMRNA.txt")
...
write.table(powerSDMat, "powerSDMatSymmSameMRNA.txt")

MSEMat <- data.frame(MSEMat)
colnames(MSEMat) <- 
  c("Oracle", "Total Count", "DESeq", "edgeR", "TCC", "PoissonSeq")

write.table(MSEMat, "MSEMatSymmSameMRNA.txt")