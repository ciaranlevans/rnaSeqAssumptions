...
n1 <- 2 # num samples in first cond.
n2 <- 2 # num samples in second cond.
...
write.table(efdrMat, "efdrMatSymmSameMRNA_prm2.txt")
...
write.table(efdrSDMat, "efdrSDMatSymmSameMRNA_prm2.txt")
...
write.table(powerMat, "powerMatSymmSameMRNA_prm2.txt")
...
write.table(powerSDMat, "powerSDMatSymmSameMRNA_prm2.txt")

MSEMat <- data.frame(MSEMat)
colnames(MSEMat) <- 
  c("Oracle", "Total Count", "DESeq", "edgeR", "TCC", "PoissonSeq")

write.table(MSEMat, "MSEMatSymmSameMRNA_prm2.txt")